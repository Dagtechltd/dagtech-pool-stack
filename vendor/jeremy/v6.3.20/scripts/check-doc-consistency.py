...
INSTALL_COMMAND = "docker compose build && docker compose up -d --no-build --pull never --no-deps postgres node dashboard"
INSTALL_COMMAND_HTML = INSTALL_COMMAND.replace("&", "&amp;")
STALE_INSTALL_RE = re.compile(
    r"docker compose build (?:&&|&amp;&amp;) docker compose up -d --no-build --pull never(?! --no-deps postgres node dashboard)"
)
