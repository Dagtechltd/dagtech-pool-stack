...
need_grep 'POOL_RPC_BACKENDS.*node=http://node:38131' "ops/release-install.sh"
...
need_grep 'compose_cmd up -d --no-build --pull never postgres node dashboard' "ops/release-install.sh"
...
if [[ "$mode" == "source" ]]; then
  ...
  need_grep 'docker compose up -d --no-build --pull never postgres node dashboard' "scripts/release/installers/install-unix-common.sh"
